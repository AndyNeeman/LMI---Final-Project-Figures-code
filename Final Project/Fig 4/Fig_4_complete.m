function Fig_4_complete
    ...
    function Fig_4a
        ...
        function c = propagation_half(A)
            c = propagator_half(1,A,0)*propagator_half(1,A,4/5*pi)*propagator_half(1,A,2/5*pi)*propagator_half(1,A,4/5*pi)*propagator_half(1,A,0)*c_half_0;
        end

        function c = propagation_one(A)
            c = propagator_one(1,A,0)*propagator_one(1,A,4/5*pi)*propagator_one(1,A,2/5*pi)*propagator_one(1,A,4/5*pi)*propagator_one(1,A,0)*c_one_0;
        end

        function c = propagation_two(A)
            c = propagator_two(1,A,0)*propagator_two(1,A,4/5*pi)*propagator_two(1,A,2/5*pi)*propagator_two(1,A,4/5*pi)*propagator_two(1,A,0)*c_two_0;
        end

        function c = propagation_four(A)
            c = propagator_four(1,A,0)*propagator_four(1,A,4/5*pi)*propagator_four(1,A,2/5*pi)*propagator_four(1,A,4/5*pi)*propagator_four(1,A,0)*c_four_0;
        end
        ...
        ylim([0, 1]);
        legend('$j = \frac{1}{2}$', '$j = 1$', '$j = 2$', '$j = 4$', 'Interpreter','latex')
    end
    ...
    function Fig_4b
        ...
        function c = propagation_half(A)
            c = propagator_half(1,A,0)*propagator_half(1,A,14/15*pi)*propagator_half(1,A,12/15*pi)*propagator_half(1,A,24/15*pi)*propagator_half(1,A,20/15*pi)* ...
                propagator_half(1,A,30/15*pi)*propagator_half(1,A,24/15*pi)*propagator_half(1,A,32/15*pi)*propagator_half(1,A,24/15*pi)*propagator_half(1,A,30/15*pi)* ...
                propagator_half(1,A,20/15*pi)*propagator_half(1,A,24/15*pi)*propagator_half(1,A,12/15*pi)*propagator_half(1,A,14/15*pi)*propagator_half(1,A,0)*c_half_0;
        end

        function c = propagation_one(A)
            c = propagator_one(1,A,0)*propagator_one(1,A,14/15*pi)*propagator_one(1,A,12/15*pi)*propagator_one(1,A,24/15*pi)*propagator_one(1,A,20/15*pi)* ...
                propagator_one(1,A,30/15*pi)*propagator_one(1,A,24/15*pi)*propagator_one(1,A,32/15*pi)*propagator_one(1,A,24/15*pi)*propagator_one(1,A,30/15*pi)* ...
                propagator_one(1,A,20/15*pi)*propagator_one(1,A,24/15*pi)*propagator_one(1,A,12/15*pi)*propagator_one(1,A,14/15*pi)*propagator_one(1,A,0)*c_one_0;
        end

        function c = propagation_two(A)
            c = propagator_two(1,A,0)*propagator_two(1,A,14/15*pi)*propagator_two(1,A,12/15*pi)*propagator_two(1,A,24/15*pi)*propagator_two(1,A,20/15*pi)* ...
                propagator_two(1,A,30/15*pi)*propagator_two(1,A,24/15*pi)*propagator_two(1,A,32/15*pi)*propagator_two(1,A,24/15*pi)*propagator_two(1,A,30/15*pi)* ...
                propagator_two(1,A,20/15*pi)*propagator_two(1,A,24/15*pi)*propagator_two(1,A,12/15*pi)*propagator_two(1,A,14/15*pi)*propagator_two(1,A,0)*c_two_0;
        end

        function c = propagation_four(A)
            c = propagator_four(1,A,0)*propagator_four(1,A,14/15*pi)*propagator_four(1,A,12/15*pi)*propagator_four(1,A,24/15*pi)*propagator_four(1,A,20/15*pi)* ...
                propagator_four(1,A,30/15*pi)*propagator_four(1,A,24/15*pi)*propagator_four(1,A,32/15*pi)*propagator_four(1,A,24/15*pi)*propagator_four(1,A,30/15*pi)* ...
                propagator_four(1,A,20/15*pi)*propagator_four(1,A,24/15*pi)*propagator_four(1,A,12/15*pi)*propagator_four(1,A,14/15*pi)*propagator_four(1,A,0)*c_four_0;
        end
    ...
    function Fig_4c
        ...
        function c = propagation_half(A)
            c = propagator_half(1,A,0)*propagator_half(1,A,1.161*pi)*propagator_half(1,A,0.580*pi)*propagator_half(1,A,1.161*pi)*propagator_half(1,A,0)*c_half_0;
        end

        function c = propagation_one(A)
            c = propagator_one(1,A,0)*propagator_one(1,A,1.161*pi)*propagator_one(1,A,0.580*pi)*propagator_one(1,A,1.161*pi)*propagator_one(1,A,0)*c_one_0;
        end

        function c = propagation_two(A)
            c = propagator_two(1,A,0)*propagator_two(1,A,1.161*pi)*propagator_two(1,A,0.580*pi)*propagator_two(1,A,1.161*pi)*propagator_two(1,A,0)*c_two_0;
        end

        function c = propagation_four(A)
            c = propagator_four(1,A,0)*propagator_four(1,A,1.161*pi)*propagator_four(1,A,0.580*pi)*propagator_four(1,A,1.161*pi)*propagator_four(1,A,0)*c_four_0;
        end
    ...
    xlabel('Pulse Area in Units of $\pi$','Interpreter','latex')
